...
public class Parser {
    ...
    public Statement parse(List<String> tokens) {
        ...
        if (command.equals("LET")) {
            ...
            Expression condition = parseExpression(tokens.subList(1, tokens.indexOf("THEN")));
            List<String> thenTokens = tokens.subList(tokens.indexOf("THEN") + 1, tokens.size());
            Statement thenStatement = parse(thenTokens);
            return new IfStatement(condition, thenStatement);
        } else if (command.equals("WHILE")) {
            Expression condition = parseExpression(tokens.subList(1, tokens.indexOf("DO")));
            List<String> bodyTokens = tokens.subList(tokens.indexOf("DO") + 1, tokens.size());
            Statement body = parse(bodyTokens);
            return new WhileStatement(condition, body);
        } else if (command.equals("FOR")) {
            String variable = tokens.get(1);
            Expression start = parseExpression(tokens.subList(3, 4));
            Expression end = parseExpression(tokens.subList(5, 6));
            List<String> bodyTokens = tokens.subList(tokens.indexOf("DO") + 1, tokens.size());
            Statement body = parse(bodyTokens);
